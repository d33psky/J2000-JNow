...
int main(int argc, char **argv) {
    ...
    result = eraDtf2d ( "UTC", utc_tm->tm_year+1900, utc_tm->tm_mon+1, utc_tm->tm_mday, utc_tm->tm_hour, utc_tm->tm_min, utc_tm->tm_sec + ((double)tv.tv_usec / 1000000), &utc1, &utc2 );
    if (result != 0) {
    ...
    double ERFA_JNow_ra  = rob * ERFA_DR2D; // Radians to degrees
    double ERFA_JNow_dec = dob * ERFA_DR2D;
    ...
    double NOVAS_ERFA_ra_das = (NOVAS_JNow_ra - ERFA_JNow_ra) * DD2R * DR2AS;
    double NOVAS_ERFA_dec_das = (NOVAS_JNow_dec - ERFA_JNow_dec) * DD2R * DR2AS;
